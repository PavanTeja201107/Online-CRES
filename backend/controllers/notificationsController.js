...
exports.getMyNotifications = async (req, res) => {
  try {
    ...
    const [rows] = await pool.query(
      'SELECT * FROM Election WHERE class_id = ? ORDER BY created_at DESC LIMIT 20',
      [classId]
    );
    const now = Date.now();
    const notices = [];
    for (const e of rows) {
      ...
      if (now >= ns && now <= ne) {
        notices.push({
          ...
          message: `Nominations open until ${new Date(ne).toLocaleString()}`,
          ts: e.nomination_start,
        });
      }
      if (now >= vs && now <= ve) {
        notices.push({
          type: 'VOTING_OPEN',
          election_id: e.election_id,
          message: `Voting open until ${new Date(ve).toLocaleString()}`,
          ts: e.voting_start,
        });
      }
      if (e.is_published && now > ve) {
        notices.push({
          ...
          election_id: e.election_id,
          message: `Results published for Election #${e.election_id}`,
