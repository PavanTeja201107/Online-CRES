...
exports.getOrCreateTokenForStudent = async (req, res) => {
  ...
  try {
    ...
    const [vsRows] = await pool.query(
      'SELECT * FROM VoterStatus WHERE student_id = ? AND election_id = ?',
      [studentId, electionId]
    );
    if (!vsRows.length) {
      return res
        .status(403)
        .json({ error: 'Student not eligible to vote in this election' });
    }
    ...
    const [tokenRows] = await pool.query(
      'SELECT token_id, used FROM VotingToken WHERE student_id = ? AND election_id = ? AND used = FALSE LIMIT 1',
      [studentId, electionId]
    );
    ...
    if (tokenRows.length && tokenRows[0].used === 0) {
      ...
      await pool.query(
        'UPDATE VotingToken SET token_hash = ? WHERE token_id = ?',
        [tokenHash, tokenRows[0].token_id]
      );
    } else {
      ...
      await pool.query(
        'INSERT INTO VotingToken (token_id, student_id, election_id, token_hash, used) VALUES (?, ?, ?, ?, ?)',
        [tokenId, studentId, electionId, tokenHash, false]
      );
    }

    await logAction(studentId, req.user.role, ip, 'TOKEN_ISSUED', {
...
exports.castVote = async (req, res) => {
  ...
  if (!token || !candidate_id || !election_id)
  ...
  try {
    ...
    if (policyRows.length) {
      ...
      if (!pa.length) {
        ...
        return res
          .status(403)
          .json({ error: 'You must accept the voting policy before voting.' });
      }
    }
    ...
    const [tokenRows] = await conn.query(
      'SELECT token_id, student_id, used FROM VotingToken WHERE token_hash = ? AND election_id = ? FOR UPDATE',
      [tokenHash, election_id]
    );
    if (!tokenRows.length) {
      await conn.rollback();
      await logAction(
        ...
        ip,
        'VOTE_FAILURE',
        { reason: 'invalid_token', election_id },
        'FAILURE'
      );
    ...
    const [vsRows] = await conn.query(
      'SELECT id, has_voted FROM VoterStatus WHERE student_id = ? AND election_id = ? FOR UPDATE',
      [tokenRow.student_id, election_id]
    );
    if (!vsRows.length) {
      await conn.rollback();
      await logAction(
        ...
        ip,
        'VOTE_FAILURE',
        { reason: 'not_eligible', election_id },
        'FAILURE'
      );
    ...
    if (vsRows[0].has_voted) {
      ...
      await logAction(
        ...
        ip,
        'VOTE_FAILURE',
        { reason: 'already_voted', election_id },
        'FAILURE'
      );
    ...
    const [eRows] = await conn.query(
      'SELECT * FROM Election WHERE election_id = ? FOR UPDATE',
      [election_id]
    );
    if (!eRows.length) {
      await conn.rollback();
    ...
    const [candRows] = await conn.query(
      'SELECT * FROM Nomination WHERE student_id = ? AND election_id = ? AND status = ?',
    ...
    await conn.query(
      'INSERT INTO VoteAnonymous (election_id, ballot_id, candidate_id) VALUES (?, ?, ?)',
      [election_id, ballotId, candidate_id]
    );

    // mark token used and anonymize mapping
    await conn.query(
      'UPDATE VotingToken SET used = TRUE, used_at = NOW(), student_id = NULL, token_hash = NULL WHERE token_id = ?',
      [tokenRow.token_id]
    );

    // update voter status (has_voted)
    await conn.query(
      'UPDATE VoterStatus SET has_voted = TRUE, voted_at = NOW() WHERE id = ?',
      [vsRows[0].id]
    );
    ...
    await logAction(
      ...
      ip,
      'VOTE_CAST',
      { election_id, ballot_id: ballotId },
      'SUCCESS'
    );
    await logAction(
      'ANON',
