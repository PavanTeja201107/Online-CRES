  ...
  try {
    ...
    if (pendingNoms.length > 0) {
      ...
      for (const nom of pendingNoms) {
        try {
          ...
          console.log(`Auto-rejection email sent to ${nom.email} for nomination_id ${nom.nomination_id}`);
        } catch (emailErr) {
          console.error(`Failed to send auto-rejection email to ${nom.email}:`, emailErr?.message || emailErr);
    ...
    for (const row of toClose) {
      try {
        ...
        console.error(`Election auto-close error for election_id ${row.election_id}:`, innerErr?.message || innerErr);
      }
    }
    ...
    for (const row of toActivate) {
      try {
        ...
        if (students.length === 0) {
          console.warn(`No students found for class_id ${row.class_id}, election_id ${row.election_id}`);
          continue;
        }
        ...
        try {
          ...
          console.error(`Election auto-activate error for election_id ${row.election_id}:`, innerErr?.message || innerErr);
        }
      } catch (err) {
