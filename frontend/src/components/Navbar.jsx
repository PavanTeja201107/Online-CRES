...
export default function Navbar(){
	...
	const linkClass = ({isActive}) => `block py-2 md:py-0 ${isActive ? 'text-indigo-600 font-semibold' : 'text-gray-700'}`;

	const AdminMenu = () => (
		<nav className="flex flex-col md:flex-row md:items-center gap-2 md:gap-4 text-sm">
		...
		<header className="bg-white border-b shadow-sm sticky top-0 z-40">
			<div className="container mx-auto p-4">
				<div className="flex items-center justify-between">
					...
					{/* Mobile menu button */}
					{user && (
						<button className="md:hidden inline-flex items-center justify-center p-2 rounded border border-gray-300 text-gray-600" onClick={()=>setOpen(o=>!o)} aria-label="Toggle menu">
				...
				{user && (
					<div className={`mt-3 md:mt-0 md:flex md:items-center md:justify-between ${open ? '' : 'hidden md:flex'}`}>
