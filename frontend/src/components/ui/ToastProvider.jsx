      ...
      <div className="fixed top-4 right-4 space-y-2 z-50">
        {toasts.map((t) =>
          t.isLastLogin ? (
            <div
              key={t.id}
              className="px-6 py-4 rounded-xl shadow-2xl bg-gradient-to-r from-indigo-600 to-purple-600 text-white max-w-md animate-slide-in border-2 border-white/20"
            >
            ...
            <div
              key={t.id}
              className={`px-4 py-2 rounded shadow text-white text-sm ${t.kind === 'success' ? 'bg-emerald-600' : t.kind === 'danger' ? 'bg-red-600' : t.kind === 'warning' ? 'bg-yellow-600' : 'bg-indigo-600'}`}
            >
              {t.message}
            </div>
          ),
