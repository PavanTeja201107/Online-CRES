...
export default function SearchableSelect({ label, options = [], value, onChange, placeholder = '-- Choose --', required, className = '', display, getValue, ...props }) {
  const [q, setQ] = useState('');
  const normalize = (s) => String(s || '').toLowerCase();
  const filtered = useMemo(() => {
    const qq = normalize(q);
    if (!qq) return options;
    return options.filter(opt => normalize(display ? display(opt) : opt?.label || opt?.name || opt?.title || opt).includes(qq));
  }, [options, q, display]);
      ...
      <div className="flex gap-2">
        ...
        >
          ...
          {filtered.map((opt, idx) => {
            const v = getValue ? getValue(opt) : (opt.value ?? opt.id ?? opt.class_id ?? opt.election_id ?? idx);
