        ...
        {activeElection && (
          <div className="bg-white p-4 rounded shadow mb-6">
            ...
            <div className="text-sm text-gray-700">
              ...
              {new Date(activeElection.nomination_start).toLocaleString()} -{' '}
              {new Date(activeElection.nomination_end).toLocaleString()}
            </div>
            <div className="text-sm text-gray-700">
              <strong>Voting:</strong> {new Date(activeElection.voting_start).toLocaleString()} -{' '}
              {new Date(activeElection.voting_end).toLocaleString()}
            </div>
            <div className="mt-2">
            ...
            <ul className="divide-y">
              {elections.map((e) => {
                ...
                if (now >= ns && now <= ne) {
                  ...
                  status = e.is_published ? 'Completed' : 'Voting Closed (Pending Results)';
                  badge = e.is_published ? 'bg-gray-700' : 'bg-orange-600';
                }
                return (
                  <li
                    key={e.election_id}
                  ...
                  >
                    <div className="space-y-1 text-sm">
                      ...
                      <div className="text-gray-600">
                        Nomination: {new Date(e.nomination_start).toLocaleString()} -{' '}
                        {new Date(e.nomination_end).toLocaleString()}
                      </div>
                      <div className="text-gray-600">
                        Voting: {new Date(e.voting_start).toLocaleString()} -{' '}
                    ...
                    <div className="mt-2 sm:mt-0">
                      <Badge
                        color={
                          badge.includes('blue')
                            ? 'blue'
                            : badge.includes('emerald')
                              ? 'emerald'
                              : badge.includes('orange')
                                ? 'orange'
                                : 'gray'
                        }
                      >
