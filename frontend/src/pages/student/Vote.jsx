...
export default function VotePage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        try {
          if (e.voting_policy_id) {
            const p = await getPolicyById(e.voting_policy_id);
            setPolicy(p);
          }
        } catch { /* optional */ }
      } catch (error) {
  ...
  const votingOpen = election ? (() => {
    const now = Date.now();
    const vs = new Date(election.voting_start).getTime();
    const ve = new Date(election.voting_end).getTime();
    return now >= vs && now <= ve;
  })() : false;
      ...
      <div className="container mx-auto px-6 py-8">
        ...
        {election && (
          <div className="bg-white p-4 rounded shadow mb-4">
            ...
            <div className="text-sm text-gray-700 mb-2">Voting window: {new Date(election.voting_start).toLocaleString()} - {new Date(election.voting_end).toLocaleString()}</div>
            <button onClick={getToken} disabled={alreadyVoted || !votingOpen} className="bg-indigo-600 text-white px-3 py-1 rounded disabled:opacity-60">
              {alreadyVoted ? 'Already Voted' : (votingOpen ? 'Get Token' : 'Voting not open')}
            </button>
          </div>
        )}
          ...
          <div className="fixed inset-0 bg-black/40 flex items-center justify-center p-4">
            <div className="bg-white rounded shadow max-w-2xl w-full p-4">
              ...
              <div className="flex justify-end gap-2">
                ...
                <button onClick={async ()=>{ try{ await acceptPolicy(policy?.policy_id); setAccepted(true); setShowPolicy(false); setMsg('Policy accepted. Please submit your vote again.'); }catch(e){ setErr(e.response?.data?.error || 'Failed to accept'); } }} className="px-3 py-1 rounded bg-indigo-600 text-white">I Accept</button>
              </div>
            </div>
          </div>
        )}
