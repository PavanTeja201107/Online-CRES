...
import { requestPasswordReset as requestResetApi, resetPassword as resetPasswordApi } from '../../api/authApi';
...
export default function AdminProfile(){
  ...
  const submitReset = async (e) => {
    e.preventDefault(); setErr(''); setMsg('');
    if (!otp || !newPw || !confirmPw) { setErr('Please fill all fields'); return; }
          ...
          <div className="grid grid-cols-1 gap-6 max-w-3xl">
            <form onSubmit={save} className="bg-white p-4 rounded shadow">
              ...
              <label className="block text-sm mb-2">Name
                <input value={name} onChange={e=>setName(e.target.value)} className="border p-2 w-full mt-1" />
              </label>
              <label className="block text-sm mb-4">Email
                <input value={email} onChange={e=>setEmail(e.target.value)} className="border p-2 w-full mt-1" />
              </label>
            ...
            <div className="bg-white p-4 rounded shadow">
              ...
              {(msg || err) && <div className={`mt-3 text-sm ${err? 'text-red-600':'text-green-600'}`}>{err || msg}</div>}
              {otpRequested && (
