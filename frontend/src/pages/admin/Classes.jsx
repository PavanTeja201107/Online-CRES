...
export default function AdminClasses(){
  ...
  const filteredClasses = useMemo(() => {
    ...
    if (!q) return classes;
    return classes.filter(c => String(c.class_id).toLowerCase().includes(q) || String(c.class_name || '').toLowerCase().includes(q));
  }, [classes, search]);
  ...
  const requestDelete = (clazz) => {
    ...
    if (count > 0) {
      ...
      // direct delete
      deleteClass(clazz.class_id).then(()=>{
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="container mx-auto px-6 py-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="divide-y">
                {filteredClasses.map(c => {
                  ...
                  return (
                    ...
                    >
                      <div>
                        <div className="font-medium">{c.class_id} - {c.class_name}</div>
          ...
          <div className="lg:col-span-2">
            {!selectedClassId ? (
              <div className="bg-white p-6 rounded shadow text-gray-600">Select a class to view enrolled students.</div>
            ) : (
                ...
                <div className="overflow-auto rounded border">
                  <table className="min-w-full divide-y">
                    <thead className="bg-gray-50">
                      <tr>
                        <th className="px-4 py-2 text-left text-xs font-semibold uppercase tracking-wide text-gray-600">Student ID</th>
                    ...
                    <tbody className="divide-y">
                      ...
                      {!selectedStudents.length && (
                        <tr>
                          <td colSpan="3" className="px-4 py-6 text-center text-gray-600">No students in this class.</td>
                        </tr>
                      )}
                    </tbody>
                  </table>
      ...
      <Modal
        ...
        footer={[
          <button key="cancel" onClick={()=>{ setConfirmOpen(false); setPendingDelete(null); }} className="px-4 py-2 rounded border">Cancel</button>,
          <button
            key="confirm"
            onClick={async ()=>{
              ...
              try {
                ...
                setMsg(`Class ${pendingDelete.class_name} deleted. ${pendingDelete.count} students and all related elections, nominations, and votes were removed.`);
                setErr('');
      ...
      >
        {pendingDelete ? (
          <div className="space-y-3">
            <p>
              The class <strong>{pendingDelete.class_name}</strong> (ID: {pendingDelete.class_id}) has <strong>{pendingDelete.count}</strong> enrolled students.
            </p>
            ...
            <p>
              All enrolled students will be notified by email (if SMTP is configured).
            </p>
